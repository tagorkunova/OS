...
void init_storage(int size) {
    storage.first = NULL;
    storage.size = size;
    pthread_mutex_init(&storage.head_lock, NULL);
...
void free_storage() {
    ...
    pthread_mutex_destroy(&storage.head_lock);
}
    ...
    while (!stop_flag) {
        ...
        if (pos == 0) {
            ...
            for (int i = 0; i < pos - 1 && curr->next; i++) {
                ...
                pthread_mutex_lock(&next_node->sync);
                pthread_mutex_unlock(&curr->sync);
                curr = next_node;
            }
...
int main(int argc, char *argv[]) {
    ...
    printf("Initializing list with %d elements (mutex version)...\n", list_size);
