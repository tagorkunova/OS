...
void init_storage(int size) {
    storage.first = NULL;
    storage.size = size;
    pthread_spin_init(&storage.head_lock, PTHREAD_PROCESS_PRIVATE);
...
void free_storage() {
    ...
    pthread_spin_destroy(&storage.head_lock);
}
...
int main(int argc, char *argv[]) {
    ...
    printf("Initializing list with %d elements (spinlock version)...\n", list_size);
